...
import { useRegisterMutation, useVeriyfUserMutation } from '../slices/usersApiSlice'
import '../css/form.css'
...
const SignupForm = () => {
  ...
  const [verifyUser] = useVeriyfUserMutation();
  ...
  const userVerification = async (e) => {
    ...
    try{
      if(userOTP === systemOTP){
        ...
        toast.error('Invalid Verification code', { autoClose: 2000 });
      }
    }catch(error){
      toast.error(error?.data?.message || error.error, { autoClose: 2000 });
    }
  ...
  const generateToken = async (e) => {
    ...
    if (data.password !== data.confirmPassword) {
      ...
      toast.error('Password must contain at least 8 characters, one number, one letter, and one symbol', { autoClose: 2000 });
      return
    } else{
    try{
      ...
      toast.warning('Please enter the verification code', { autoClose: 2000 });
    }catch(error){
      toast.error(error?.data?.message || error.error, { autoClose: 2000 });
    }}
  }
  ...
  return (
    <div className="flex text-n-8">
      <div className="flex flex-col w-[100%] lg:w-[50%] items-center justify-center relative z-1 bg-n-1 py-[4rem]">
        <Link to='/' className="w-[80%] lg:w-[60%]">
        ...
        <form onSubmit={generateToken} className="flex flex-col w-[80%] lg:w-[60%] mt-4">
          ...
          <button className="button text-n-1 bg-sky-700 h-[45px] mt-2 rounded-md border border-sky-700 hover:text-sky-700 hover:bg-transparent">Sign Up</button>
        </form>
        {verificationField && (
          <div className="fixed inset-0 flex justify-center items-center bg-n-8 bg-opacity-10">
            <div className='fixed my-auto mx-auto bg-n-1 text-n-8  shadow rounded-xl flex flex-col p-7 items-center'>
              <div className="flex flex-col items-center">
                ...
                <h1 className="h5 font-bold mt-4">Enter Verification Code</h1>
              </div>
              <form onSubmit={userVerification} className="bg-n-1 rounded-xl flex flex-col items-center">
                <div className="py-6">
                {otpFields.map((item, index) => (
                  <input 
                    ...
                    onKeyDown={(e) => {
                      ...
                      } else if (e.key >= '0' && e.key <= '9' && index < otpFields.length - 1) {
                ...
                <button type="submit" className="bg-sky-600 w-full py-2 text-n-1 font-bold tracking-wider rounded-md hover:bg-sky-700">Verify</button>
              </form>
            </div>
          </div>
        )}
